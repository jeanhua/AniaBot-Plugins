...
- 发送机制：插件先把原图下载到本地（带 Referer+UA，`image_proxy` 换源依然有效）再转 base64 发送，不再直发 URL——直发是让 QQ/TG 等服务端二次抓图床，不带 Referer 易 403 失败。单张限 20MB、30 秒超时，超限/超时会明确提示。
- 逐张发送、逐张报错：下载失败会告诉你哪张、为什么（如 403/404/超时），并附作品页链接兜底；图已下载但 Bot 返回失败也会单独提示（多为网络波动、风控或图片过大），不再静默失败。
